...
function InputController() {
  ...
  const populateEditTaskModal = (projects, projectIndex, taskIndex) => {
    ...
    if (!task) {
      console.error(
        `No task found at index ${taskIndex} for project ${projectIndex}`
      );
      return;
    }
    document.querySelector(
      ".editHeader"
    ).textContent = `Project: ${projects[projectIndex].title} `;
    document.querySelector("#editTaskName").value = task.name;
    document.querySelector("#editDescription").value = task.description;
    document
      .querySelectorAll('input[name="edit-priority"]')
      .forEach((priority) => {
  ...
  const updateTime = () => {
    const now = new Date();
    document.querySelector(".currentDateTime").textContent = format(
      now,
      "PPpp"
    );
  };
  ...
  const lookForDeadline = () => {
    const todayProjectIndex = listOfProjects.findIndex(
  ...
  editSubmitTaskDialog.onclick = () => {
    listOfProjects[getCurrentIndex()].TASKLIST[getCurrentTaskIndex()].name =
      document.querySelector("#editTaskName").value;
    listOfProjects[getCurrentIndex()].TASKLIST[
      getCurrentTaskIndex()
    ].description = document.querySelector("#editDescription").value;

    const priorityLevels = document.querySelectorAll(
    ...
    if (dateInput) {
      ...
      listOfProjects[getCurrentIndex()].TASKLIST[
        getCurrentTaskIndex()
      ].deadline = format(parseDate, "MMMM dd, yyyy");
    }
    LIST.saveToLocalStorage();
  ...
  taskDialog.addEventListener("close", (e) => {
    ...
    if (name && description && priority && date) {
      console.log(
        "Task Dialog closed, adding task to project index:",
        getCurrentIndex()
      );
      LIST.createTaskInstance(
        getCurrentIndex(),
        name,
        description,
        priority,
      ...
      priorityLevels.forEach(
        (priorityLevel) => (priorityLevel.checked = false)
      );
      printProjectsOnScreen();
      showMainDivContent(listOfProjects[getCurrentIndex()]);
    }
  ...
  deleteTaskYes.addEventListener("click", (e) => {
    ...
    console.log(
      `Deleting Task: '${
        listOfProjects[getCurrentIndex()].TASKLIST[getCurrentTaskIndex()].name
      }' in Project: '${listOfProjects[getCurrentIndex()].title};' `
    );
  ...
  deleteProjectYes.addEventListener("click", (e) => {
    ...
    console.log(
      `Deleting Project: ${
        listOfProjects[getCurrentIndex()].title
      } at index: ${getCurrentIndex()}`
    );
